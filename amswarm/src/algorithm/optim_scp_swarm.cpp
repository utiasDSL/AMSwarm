...
#include "qpOASES.hpp"
#include "amswarm/algorithm/optim_scp_swarm.hpp"
...
namespace Optim
{
	...
	int computeXYZ(probData &prob_data, int VERBOSE){
		...
		if(prob_data.use_thrust_values){
			...
			b_ax_ineq = Optim :: stack(prob_data.acc_max * Eigen :: ArrayXXf :: Ones(prob_data.num, 1), prob_data.acc_max * Eigen :: ArrayXXf :: Ones(prob_data.num, 1), 'v');
			b_ay_ineq = Optim :: stack(prob_data.acc_max * Eigen :: ArrayXXf :: Ones(prob_data.num, 1), prob_data.acc_max * Eigen :: ArrayXXf :: Ones(prob_data.num, 1), 'v');
			b_az_ineq = Optim :: stack(prob_data.acc_max * Eigen :: ArrayXXf :: Ones(prob_data.num, 1), -z_acc_min * Eigen :: ArrayXXf :: Ones(prob_data.num, 1), 'v');
	...
	int computeXYZ2(probData &prob_data, int VERBOSE){
		...
		Eigen :: ArrayXXf A_ineq, b_ineq_upper, b_ineq_lower;
		// @ INEQUALITY MATRICES
		
		// Positional bounds
		Eigen :: ArrayXXf A_pos_ineq = block_diag(prob_data.P, block_diag(prob_data.P, prob_data.P));
		Eigen :: ArrayXXf A_vel_ineq = block_diag(prob_data.Pdot, block_diag(prob_data.Pdot, prob_data.Pdot));
		Eigen :: ArrayXXf A_acc_ineq = block_diag(prob_data.Pddot, block_diag(prob_data.Pddot, prob_data.Pddot));
		
		Eigen :: ArrayXXf b_x_ineq_upper = prob_data.x_max * Eigen :: ArrayXXf :: Ones(prob_data.num, 1);
		Eigen :: ArrayXXf b_y_ineq_upper = prob_data.y_max * Eigen :: ArrayXXf :: Ones(prob_data.num, 1);
		Eigen :: ArrayXXf b_z_ineq_upper = prob_data.z_max * Eigen :: ArrayXXf :: Ones(prob_data.num, 1);
		Eigen :: ArrayXXf b_x_ineq_lower = prob_data.x_min * Eigen :: ArrayXXf :: Ones(prob_data.num, 1);
		Eigen :: ArrayXXf b_y_ineq_lower = prob_data.y_min * Eigen :: ArrayXXf :: Ones(prob_data.num, 1);
		Eigen :: ArrayXXf b_z_ineq_lower = prob_data.z_min * Eigen :: ArrayXXf :: Ones(prob_data.num, 1);

		Eigen :: ArrayXXf b_pos_ineq_upper = stack(stack(b_x_ineq_upper, b_y_ineq_upper, 'v'), b_z_ineq_upper,'v'); 
		Eigen :: ArrayXXf b_pos_ineq_lower = stack(stack(b_x_ineq_lower, b_y_ineq_lower, 'v'), b_z_ineq_lower,'v'); 

		// Velocity bounds
		Eigen :: ArrayXXf b_vx_ineq_upper = prob_data.vel_max * Eigen :: ArrayXXf :: Ones(prob_data.num, 1);
		Eigen :: ArrayXXf b_vy_ineq_upper = prob_data.vel_max * Eigen :: ArrayXXf :: Ones(prob_data.num, 1);
		Eigen :: ArrayXXf b_vz_ineq_upper = prob_data.vel_max * Eigen :: ArrayXXf :: Ones(prob_data.num, 1);
		Eigen :: ArrayXXf b_vx_ineq_lower = -prob_data.vel_max * Eigen :: ArrayXXf :: Ones(prob_data.num, 1);
		Eigen :: ArrayXXf b_vy_ineq_lower = -prob_data.vel_max * Eigen :: ArrayXXf :: Ones(prob_data.num, 1);
		Eigen :: ArrayXXf b_vz_ineq_lower = -prob_data.vel_max * Eigen :: ArrayXXf :: Ones(prob_data.num, 1);

		Eigen :: ArrayXXf b_vel_ineq_upper = stack(stack(b_vx_ineq_upper, b_vy_ineq_upper, 'v'), b_vz_ineq_upper,'v'); 
		Eigen :: ArrayXXf b_vel_ineq_lower = stack(stack(b_vx_ineq_lower, b_vy_ineq_lower, 'v'), b_vz_ineq_lower,'v'); 

		Eigen :: ArrayXXf b_ax_ineq_upper = prob_data.acc_max * Eigen :: ArrayXXf :: Ones(prob_data.num, 1);
		Eigen :: ArrayXXf b_ay_ineq_upper = prob_data.acc_max * Eigen :: ArrayXXf :: Ones(prob_data.num, 1);
		Eigen :: ArrayXXf b_az_ineq_upper = prob_data.acc_max * Eigen :: ArrayXXf :: Ones(prob_data.num, 1);
		Eigen :: ArrayXXf b_ax_ineq_lower = -prob_data.acc_max * Eigen :: ArrayXXf :: Ones(prob_data.num, 1);
		Eigen :: ArrayXXf b_ay_ineq_lower = -prob_data.acc_max * Eigen :: ArrayXXf :: Ones(prob_data.num, 1);
		Eigen :: ArrayXXf b_az_ineq_lower = -prob_data.acc_max * Eigen :: ArrayXXf :: Ones(prob_data.num, 1);

		if(prob_data.use_thrust_values){
			prob_data.acc_max = (-(2*prob_data.gravity) + sqrt(pow(2*prob_data.gravity,2) - 4*3*(pow(prob_data.gravity,2) - pow(prob_data.f_max*prob_data.gravity,2))))/6.0; 
			float z_acc_min = prob_data.f_min - prob_data.gravity;
			
			b_ax_ineq_upper = prob_data.acc_max * Eigen :: ArrayXXf :: Ones(prob_data.num, 1);
			b_ay_ineq_upper = prob_data.acc_max * Eigen :: ArrayXXf :: Ones(prob_data.num, 1);
			b_az_ineq_upper = prob_data.acc_max * Eigen :: ArrayXXf :: Ones(prob_data.num, 1);
			b_ax_ineq_lower = -prob_data.acc_max * Eigen :: ArrayXXf :: Ones(prob_data.num, 1);
			b_ay_ineq_lower = -prob_data.acc_max * Eigen :: ArrayXXf :: Ones(prob_data.num, 1);
			b_az_ineq_lower = z_acc_min * Eigen :: ArrayXXf :: Ones(prob_data.num, 1);

		}
		Eigen :: ArrayXXf b_acc_ineq_upper = stack(stack(b_ax_ineq_upper, b_ay_ineq_upper, 'v'), b_az_ineq_upper,'v'); 
		Eigen :: ArrayXXf b_acc_ineq_lower = stack(stack(b_ax_ineq_lower, b_ay_ineq_lower, 'v'), b_az_ineq_lower,'v'); 

		// Eigen :: ArrayXXf b_acc_ineq = stack(stack(b_ax_ineq, b_ay_ineq, 'v'), b_az_ineq, 'v');

		Eigen :: ArrayXXf A_slack_ineq, b_slack_ineq_upper, b_slack_ineq_lower;
		// @ EQUALITY MATRICES
		prob_data.A_eq = block_diag(block_diag(stack(prob_data.P.row(0), stack(prob_data.Pdot.row(0), prob_data.Pddot.row(0), 'v'), 'v'), stack(prob_data.P.row(0), stack(prob_data.Pdot.row(0), prob_data.Pddot.row(0), 'v'), 'v')), stack(prob_data.P.row(0), stack(prob_data.Pdot.row(0), prob_data.Pddot.row(0), 'v'), 'v'));
		prob_data.b_eq = Eigen :: ArrayXXf(9, 1);
		prob_data.b_eq << prob_data.x_init, prob_data.vx_init, prob_data.ax_init,
		...
		while(tries < 1 && prob_data.qp_fail){		
			...
			// 		b_pos_ineq_lower = stack(b_pos_ineq_lower, Eigen::ArrayXXf::Zero(prob_data.num*prob_data.num_static_obs, 1),'v');
			// 		b_vel_ineq_lower = stack(b_vel_ineq_lower, Eigen::ArrayXXf::Zero(prob_data.num*prob_data.num_static_obs, 1),'v');
			// 		b_acc_ineq_lower = stack(b_acc_ineq_lower, Eigen::ArrayXXf::Zero(prob_data.num*prob_data.num_static_obs, 1),'v');

			// 		b_pos_ineq_upper = stack(b_pos_ineq_upper, Eigen::ArrayXXf::Zero(prob_data.num*prob_data.num_static_obs, 1),'v');
			// 		b_vel_ineq_upper = stack(b_vel_ineq_upper, Eigen::ArrayXXf::Zero(prob_data.num*prob_data.num_static_obs, 1),'v');
			// 		b_acc_ineq_upper = stack(b_acc_ineq_upper, Eigen::ArrayXXf::Zero(prob_data.num*prob_data.num_static_obs, 1),'v');

			// 		b_slack_ineq_lower = Eigen :: ArrayXXf :: Zero(A_slack_ineq.rows(), 1); 
			// 		b_slack_ineq_upper = Eigen :: ArrayXXf :: Ones(A_slack_ineq.rows(), 1) * std::numeric_limits<float>::max(); 

			// 		prob_data.b_eq = stack(prob_data.b_eq, Eigen::ArrayXXf::Zero(prob_data.num*prob_data.num_static_obs, 1),'v');
			// 	}
			// 	Eigen :: ArrayXXf b_coll_upper = prob_data.b_coll;
			// 	Eigen :: ArrayXXf b_coll_lower = Eigen :: ArrayXXf :: Ones(prob_data.b_coll.rows(), prob_data.b_coll.cols()) * std::numeric_limits<float>::max();

			// 	A_ineq = stack(stack(stack(A_pos_ineq, A_vel_ineq, 'v'), stack(A_acc_ineq, prob_data.A_coll, 'v'), 'v'), A_slack_ineq, 'v');
			// 	b_ineq_upper = stack(stack(stack(b_pos_ineq_upper, b_vel_ineq_upper, 'v'), stack(b_acc_ineq_upper, b_coll_upper, 'v'), 'v'), b_slack_ineq_upper, 'v');
			// 	b_ineq_lower = stack(stack(stack(b_pos_ineq_upper, b_vel_ineq_lower, 'v'), stack(b_acc_ineq_upper, b_coll_lower, 'v'), 'v'), b_slack_ineq_upper, 'v');
				
			// 	temp_cost = block_diag(prob_data.weight_goal*prob_data.cost_goal + prob_data.weight_smoothness*prob_data.cost_smoothness, prob_data.weight_quad_slack*prob_data.cost_slack);
			// 	temp_lincost = stack(-prob_data.weight_goal * prob_data.lincost_goal, prob_data.weight_lin_slack * Eigen::ArrayXXf::Ones(prob_data.num*prob_data.num_static_obs, 1), 'v');
			// }
			// else{
				A_ineq = stack(stack(A_pos_ineq, A_vel_ineq, 'v'), A_acc_ineq, 'v');
				b_ineq_upper = stack(stack(b_pos_ineq_upper, b_vel_ineq_upper, 'v'), b_acc_ineq_upper, 'v');
				b_ineq_lower = stack(stack(b_pos_ineq_lower, b_vel_ineq_lower, 'v'), b_acc_ineq_lower, 'v');

				temp_cost = prob_data.weight_goal*prob_data.cost_goal + prob_data.weight_smoothness*prob_data.cost_smoothness;
				temp_lincost = -prob_data.weight_goal * prob_data.lincost_goal;
			// }

			int num_inequalities = b_ineq_lower.rows();
			int num_equalities = prob_data.b_eq.rows();
			int num_vars = temp_cost.rows();
			int num_constraints = num_inequalities + num_equalities;
			Eigen :: VectorXd _solution = Eigen::VectorXd::Zero(num_vars);
			...
			Eigen :: VectorXd bin_lower = Eigen :: VectorXd::Zero(num_constraints);
			bin_lower << b_ineq_lower.cast<double>().matrix(), prob_data.b_eq.cast<double>().matrix();

			Eigen :: VectorXd bin_upper = Eigen :: VectorXd::Zero(num_constraints);
			bin_upper << b_ineq_upper.cast<double>().matrix(), prob_data.b_eq.cast<double>().matrix();

			Eigen :: MatrixXd Ain = Eigen :: MatrixXd::Zero(num_constraints, num_vars);
			Ain << A_ineq.cast<double>().matrix(), prob_data.A_eq.cast<double>().matrix();

			Eigen :: VectorXd f = temp_lincost.cast<double>();
			// Matrices Ain and H need to be in Rowmajor form for qpOASES
			Eigen :: MatrixXd H =  2*temp_cost.cast<double>().matrix();
			...
			qpOASES::real_t cpu_time = 0.05;
			qpOASES::returnValue status = qp.init(H_row.data(), f.data(), Ain_row.data(),
			...
			Eigen :: ArrayXXf sol = _solution.cast<float>();
			Eigen :: ArrayXXf sol_xy = sol.topRows(2*prob_data.nvar);
			Eigen :: ArrayXXf sol_xyz = sol.topRows(3*prob_data.nvar);
	...
	void deployAgent(probData &prob_data, int VERBOSE){
		...
		if(prob_data.solver=="quadprog")
			...
			Optim :: computeXYZ2(prob_data, VERBOSE);
